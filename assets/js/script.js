...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!enablePointerEffects) {
    interactiveButtons.forEach((btn) => {
      ...
      btn.style.removeProperty("--border-angle");
    });
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const words = ["marketing", "ecommerce", "content idea", "brand growth"];
  let wordIndex = 0;
...
document.addEventListener("DOMContentLoaded", () => {
  const featureCards = document.querySelectorAll(
    "#features .glass-panel, #manual-research-section .glass-panel",
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!gridTorch || !hasFinePointer) return;
    ...
    newsletterBtn.addEventListener("click", async () => {
      ...
      try {
        const response = await fetch(
          "https://conthunt-976912795426.us-central1.run.app/v1/waitlist",
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          ...
          const successText = isAlreadyJoined
            ? "You're already on the list!"
            : "You're on the list!";
          const icon = isAlreadyJoined
            ? "lucide:check-circle"
            : "lucide:sparkles";
